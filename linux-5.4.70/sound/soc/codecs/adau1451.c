...
#define pr_fmt(fmt) "ADAU1451" FORMAT(fmt)
...
#define ADAU1451_SOFT_RESET (0x00)
#define ADAU1451_ADC_DAC_STATUS (0x01)
...
static bool adau1451_register_volatile(struct device *dev, unsigned int reg) {
	switch (reg) {
	case ADAU1451_SOFT_RESET:
	case ADAU1451_ADC_DAC_STATUS:
		return true;
	default:
		return false;
	}
...
static const struct regmap_config adau1451_remap_config = {
	.val_bits = 8,
	.reg_bits = 8,
...
static int adau1451_hw_params(struct snd_pcm_substream *substream,
	struct snd_pcm_hw_params *params, struct snd_soc_dai *dai) {
	pr_debug("enter");
	return 0;
}

static int adau1451_set_dai_fmt(struct snd_soc_dai *dai, unsigned int fmt) {
	pr_debug("enter");
	return 0;
}

static int adau1451_set_dai_sysclk(struct snd_soc_dai *dai,
	int clk_id, unsigned int freq, int dir) {
	pr_debug("enter");
	return 0;
}

static const struct snd_soc_dai_ops adau1451_dai_ops = {
	.hw_params	= adau1451_hw_params,
	.set_sysclk	= adau1451_set_dai_sysclk,
	.set_fmt	= adau1451_set_dai_fmt,
};
...
static int adau1451_i2c_probe(struct i2c_client *client,
	...
	return devm_snd_soc_register_component(&client->dev,
				...
				adau1451_dai_driver,
                ARRAY_SIZE(adau1451_dai_driver));
}
